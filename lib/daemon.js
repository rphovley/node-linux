...
var daemon = function(config) {
     ...
     * Alternatively, `systemd' or 'user-systemd` can be used.
     */
    mode: {
      enumerable: true,
      writable: true,
     ...
     * May return either 'systemv', 'systemd', or 'user-systemd'.
     */
    defaultMode: {
      enumerable: false,
      get: function(){
        ...
        var hasUpstart = function() {
          return false; // Upstart support is not implemented
        }
        ...
        if (hasSystemD()) {
          return this.user === 'root' ? 'systemd' : 'user-systemd';
        } else if (hasUpstart()) {
          return 'upstart';
        } else if (hasSystemV()) {
          return 'systemv';
    ...
    /**
     * @cfg {String} [user=process.env.USER]
     * The user to run the service as. Defaults to the current user
     */
    user: {
      enumerable: true,
      writable: false,
      configurable: false,
      value: config.user || process.env.USER
    },

    /**
     * @cfg {String} [group=process.env.USER]
     * The group to run the service as. Defaults to the current user's group
     */
    group: {
      enumerable: true,
      writable: false,
      configurable: false,
      value: config.group || process.env.USER
    },
    ...
    piddir: {
      ...
      writable: true,
      configurable: false,
      value: config.piddir || (this.user === 'root' ? '/var/run' : p.join(require('os').homedir(), '.local', 'run'))
    },
    ...
    logpath: {
        ...
        writable: true,
        configurable: false,
        value: config.logpath || (this.user === 'root' ? '/var/log' : p.join(require('os').homedir(), '.local', 'log'))
    },
  ...
  if (!this.mode) {
    this.mode = this.user === 'root' ? this.defaultMode : 'user-systemd';
    console.log("Using default mode:", this.mode);
  } else {
    console.log("Using mode:", this.mode);
  }
  if (['systemv','systemd','user-systemd'].indexOf(this.mode) < 0){
    console.warn("Invalid mode %s, using user-systemd instead", this.mode);
    this.mode = 'user-systemd';
  }
